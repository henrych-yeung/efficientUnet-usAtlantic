...
def image_standardize(im, axis = (0,1), c = 1e-8):
    '''
    Standardize image to zero mean and unit standard deviation along the given axis'''
    return (im - im.mean(axis)) / (im.std(axis) + c)
   
def image_normalize(im, axis = (0,1), c = 1e-8):
    '''
    Normalize image to [0, 1] range'''
    return (im - np.min(im, axis=axis)) / (np.ptp(im, axis=axis)+c)

   
def image_scale(im, axis = (0,1), c = 1e-8):
    '''
    Normalize image to [0, 1] range'''
    return im / 255
...
class FrameInfo:
    ...
    def getPatch(self, i, j, patch_size, img_size, normalize=1.0):
        ...
        if normalize >= r[0]:
#             im = image_standardize(im, axis=(0, 1))
